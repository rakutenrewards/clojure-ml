...
(defn- n-trees
  [model]
  (let [dummy-data (->predict-DMatrix [0.0])
        max-number-of-trees Integer/MAX_VALUE]
    (count (first (.predictLeaf model dummy-data max-number-of-trees)))))
    ...
    (->
     ...
     (locking model
       ...
       (if (= 0 (get-xgboost-handle model))
         ...
         (.predict model dmatrix false (n-trees model))))
     (ffirst))))
#+END_SRC
