...
(ns curbside.ml.regression.xgboost
  ...
  (:require
   ...
   [curbside.ml.interface :as interface]
...
(defmethod interface/train-model :xgboost
  [_ prob params]
  (train prob params))
#+END_SRC
...
(defmethod interface/dispose :xgboost
  [_ model]
  (dispose model))
#+END_SRC
...
(defn- predict
  [model config feature-vector]
  (let [booster (get-in config [:train-models :hyperparameters :booster])
        num-trees (get-in config [:train-models :hyperparameters :num-rounds])
...
(defmethod interface/predict :xgboost
...
#+BEGIN_SRC clojure
(defmethod interface/save-model :xgboost
  [_ model file]
  (.saveModel model file)
  [file])

(defmethod interface/load-model :xgboost
  [_ file]
  (XGBoost/loadModel file))

(defmethod interface/load-model-from-bytes :xgboost
  [_ bytes]
