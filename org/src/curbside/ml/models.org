...
(defn- train-test-split-validate
  "Evaluate the performance of a model using a held out test set validation.
  Takes a training set CSV file as input and the percentage of data to be used
  for training the model, rest of the data is used for validating."
  [algorithm predictor-type selected-features hyperparameters training-set-path
   {:as _options :keys [train-split-percentage scaling-factors feature-scaling-fns
...
(defn evaluate
  ...
  [algorithm predictor-type selected-features hyperparameters training-set-path evaluate-options
   & {:keys [_multithreads _scaling-factors _feature-scaling-fns _label-scaling-fns _example-weights-path] :as options}]
  (let [evaluate-type (:type evaluate-options)]
    (case evaluate-type
      :train-test-split (train-test-split-validate algorithm
                                                   predictor-type
                                                   selected-features
                                                   hyperparameters
                                                   training-set-path
                                                   (into options evaluate-options))
      :k-fold (cross-validate algorithm
                  predictor-type
                  selected-features
                  hyperparameters
                  training-set-path
                  (assoc options :k-folds (:folds evaluate-options)
                  :multithreads (:multithreads evaluate-options))))))
#+END_SRC
