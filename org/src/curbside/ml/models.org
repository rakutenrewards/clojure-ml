...
(deftest test-optimize-hyperparameters-grid
  (testing "Check if optimize hyperparameters returns a model with all valid sets of hyperparameters according to given spec or not for grid search"
    (let [hyperparameters {:eval_metric "mae" :booster "dart"}
      ...
      (and (is (tutils/approx= 0.6 (optimal-params :subsample) 1e-1))
      (is (tutils/approx= 7 (optimal-params :max_depth) 1e-1))
      (is (tutils/approx= 0.02851003408432007 (model-evaluations :mean-absolute-error) 1e-1))
      (is (tutils/approx= 0.02851003408432007 (model-evaluations :root-mean-square-error) 1e-1))))))


(deftest test-optimize-hyperparameters-random
  (testing "Check if optimize hyperparameters returns a model with all valid sets of hyperparameters according to given spec or not for random search"
    (stubbing [models/random-search-combos [{:subsample 0.5643362797872074, :max_depth 6, :booster "dart"}
              ...
              (let [hyperparameters {:eval_metric "mae" :booster "dart"}
                    hyperparameter-search-space-random {:subsample {:min  0.5
                                                                    :max  0.99
                                                                    :type "decimal"}
                                                        :max_depth {:min  5
                                                                    :max  9
                    ...
                    {:keys [optimal-params model-evaluations]} (models/optimize-hyperparameters :xgboost
                                                                                                ...
                                                                                                tutils/dummy-regression-single-label-training-set-path)]
                (and (is (tutils/approx= 0.9486528438903652 (optimal-params :subsample) 1e-1))
                      (is (tutils/approx= 6 (optimal-params :max_depth) 1e-1))
                      (is (tutils/approx= 0.022209078073501587 (model-evaluations :mean-absolute-error) 1e-1))
                      (is (tutils/approx= 0.022209078073501587 (model-evaluations :root-mean-square-error) 1e-1)))))))
#+END_SRC
