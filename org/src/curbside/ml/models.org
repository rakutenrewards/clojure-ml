...
(defn evaluate
  ...
  (let [multithreads (:multithreads evaluate-options)
        splits-to-evaluate (create-train-validate-splits evaluate-options training-set-path example-weights-path)
        models-and-metrics (if multithreads
                             (let [functions (for [current-split splits-to-evaluate]
        ...
        find-best (if (= (metrics/comparator selection-metric) <)
                    min-key
                    max-key)
        individual-model-metrics (for [model-and-metric models-and-metrics]
        ...
        cumulative-metrics (reduce (fn [agg [abs-error square-error n predictions]]
                                   ...
                                   {:abs-error    0
                                    ...
                                    :n            0
                                    :predictions  []}
                                   (for [result-map models-and-metrics]
                                     [(:abs-error result-map)
                                      (:square-error result-map)
                                      (:n result-map)
                                      (:predictions result-map)]))]
    (doseq [result-map (remove #(= % best-evaluation) individual-model-metrics)]
      (dispose algorithm (:model result-map)))
